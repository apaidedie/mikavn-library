...
- Library bulk selection for applying play status, favorite, hidden flags, and collection membership to the current filtered set
...
- Batch metadata matching with queue search/provider-gap filters, candidate review, result status/write filters, filtered bulk writeback, stored job/result status, and user-confirmed writeback
...
- Dashboard task summary plus task center with queue progress overview, status/type filters, expandable logs, and retry for supported failed/cancelled operations
...
- Duplicate game merge assistance in Maintenance with recommended keep-target hints before safe merge preview
