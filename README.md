...
- Maintenance recent-task overview for batch matching, description image repair, artwork repair, and duplicate ID audit jobs with status counts, progress, timing, and log shortcuts
...
- Dashboard task summary, Maintenance task overview, and task center with queue progress overview, elapsed/remaining time estimates, text search, status/type filters, searchable expandable logs, and retry for supported failed/cancelled operations
...
- Local app-data diagnostics, searchable/filterable image-reference audit details, diagnostic log preview/pruning, old database-backup cleanup, and safe database restore scheduling with next-start protection backup
