...
- `npm run smoke:real-install:update` is the explicit real installed-app overwrite check for `E:\MikaVN Library`: it refuses to run while MikaVN is active, creates a verified database backup under `app-data/database-backups/manual-install-smoke`, runs the NSIS installer against the real install directory, launches the app, and verifies game, asset, and image counts are preserved. Public release handoff also requires a `Lower-version updater rehearsal` note that names the previous version, confirms the in-app updater reached the current version, and verifies app-data after restart.
