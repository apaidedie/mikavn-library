...
foreach ($token in @("npm run test:release-scripts", "npm run test:playwright-scripts", "npm run test:updater-release", "npm run test:diagnostic-export", "npm run test:data-safety", "npm run test:library-performance")) {
  if (!$ciWorkflow.Contains($token)) {
