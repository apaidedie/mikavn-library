...
foreach ($token in @("scripts/release/run-release-validation.ps1", "-StrictGitHubLinks", "-SkipBrowserSmoke", "-SkipLargeSmoke", "-SkipTauriBuild", "-SkipInstallSmoke", "-SkipPortableDataSmoke", "-SkipRealDataSmoke", "-SkipDesktopSmoke")) {
...
foreach ($token in @("scripts/release/check-signing-certificate.cjs")) {
  $scriptText = [string]$package.scripts.'release:signing:certificate:check'
  if (!$scriptText.Contains($token)) {
    throw "package.json release:signing:certificate:check must include token '$token'."
  }
}
foreach ($token in @("scripts/release/check-signing-certificate.cjs", "--require-certificate")) {
  $scriptText = [string]$package.scripts.'release:signing:certificate:require'
  if (!$scriptText.Contains($token)) {
    throw "package.json release:signing:certificate:require must include token '$token'."
  }
}
...
foreach ($token in @("scripts/release/check-release-handoff.cjs", "--require-public-ready")) {
  $scriptText = [string]$package.scripts.'release:handoff:require-public'
  if (!$scriptText.Contains($token)) {
...
foreach ($token in @("scripts/release/check-build-chunks.test.cjs", "scripts/release/check-source-size.test.cjs", "scripts/release/check-release-handoff.test.cjs", "scripts/release/prepare-updater-handoff.test.cjs", "scripts/release/check-signing-certificate.test.cjs", "scripts/updater-release-config.test.cjs")) {
...
foreach ($token in @("scripts/startup-database-backup.test.cjs", "scripts/settings-local-data-section.test.cjs", "scripts/real-app-data-readonly-smoke.test.cjs")) {
  $scriptText = [string]$package.scripts.'test:data-safety'
  if (!$scriptText.Contains($token)) {
    throw "package.json test:data-safety must include token '$token'."
  }
}
foreach ($token in @("scripts/maintenance-image-health.test.cjs")) {
  $scriptText = [string]$package.scripts.'test:maintenance-image-health'
  if (!$scriptText.Contains($token)) {
    throw "package.json test:maintenance-image-health must include token '$token'."
  }
}
foreach ($token in @("scripts/cover-image-performance.test.cjs", "scripts/library-filter-performance.test.cjs")) {
  $scriptText = [string]$package.scripts.'test:library-performance'
  if (!$scriptText.Contains($token)) {
    throw "package.json test:library-performance must include token '$token'."
  }
}
...
foreach ($token in @("npm run test:release-scripts", "npm run test:playwright-scripts", "npm run test:updater-release", "npm run test:diagnostic-export", "npm run test:data-safety", "npm run test:maintenance-image-health", "npm run test:library-performance")) {
  if (!$ciWorkflow.Contains($token)) {
...
foreach ($token in @("npm run release:check:strict", "npm run test:release-scripts", "npm run test:playwright-scripts", "npm run test:updater-release", "npm run test:diagnostic-export", "npm run test:data-safety", "npm run test:maintenance-image-health", "npm run test:library-performance", "cargo fmt --check", "cargo clippy -- -D warnings", "npm run tauri:build", "npm run smoke:install", "npm run smoke:portable-data", "npm run smoke:desktop", "desktop-smoke-report", "output/clean-install-smoke/**", "output/portable-app-data-smoke/**", "output/desktop-smoke/**")) {
...
foreach ($token in @("npm run release:validate:strict", "npm run release:check:strict", "npm run test:release-scripts", "npm run test:playwright-scripts", "npm run test:updater-release", "npm run test:diagnostic-export", "npm run test:data-safety", "npm run test:maintenance-image-health", "npm run test:library-performance", "npm run build", "cargo fmt --check", "cargo clippy -- -D warnings", "cargo test", "npm run tauri:build", "npm run smoke:install", "npm run smoke:portable-data", "npm run smoke:real-data:readonly", "npm run smoke:desktop", "npm run smoke:browser", "npm run smoke:large")) {
