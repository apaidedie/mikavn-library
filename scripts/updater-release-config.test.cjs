...
test('package scripts and dependencies include updater gates and frontend plugins', () => {
  ...
  assert.match(pkg.scripts['test:diagnostic-export'], /scripts\/startup-diagnostics\.test\.cjs/);
...
test('release workflow requires signing secrets and publishes updater assets', () => {
  ...
  assert.match(workflow, /npm run test:maintenance-image-health/);
...
test('release metadata gate knows about updater release checks', () => {
  ...
  assert.match(gate, /test:maintenance-image-health/);
...
test('release signing certificate preflight is wired into package and metadata gates', () => {
  ...
  assert.match(releaseValidation, /npm run release:signing:certificate:check/);
});
...
test('ci and local release validation run updater-specific tests', () => {
  ...
  assert.match(ciWorkflow, /npm run test:maintenance-image-health/);
  ...
  assert.match(releaseValidation, /npm run test:maintenance-image-health/);
