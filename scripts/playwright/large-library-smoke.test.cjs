...
test('large library smoke measures topbar quick search under a dedicated budget', () => {
  const source = fs.readFileSync(sourcePath, 'utf8');

  assert.match(source, /MIKAVN_LARGE_LIBRARY_QUICK_SEARCH_BUDGET_MS/);
  assert.match(source, /quickSearchBudgetMs/);
  assert.match(source, /report\.timings\.quickSearchMs/);
  assert.match(source, /getByRole\('textbox', \{ name: '搜索游戏' \}\)/);
  assert.match(source, /large-library-quick-search\.png/);
});
...
test('large library smoke history appends compact performance entries and reports deltas', () => {
  ...
  const firstReport = {
    gameCount: 4500,
    timings: { libraryLoadMs: 900, detailSwitchMs: 70, quickSearchMs: 210, searchMs: 800 },
    renderedRows: { initial: 240, afterLoadMore: 480 },
  };
  const secondReport = {
    gameCount: 4500,
    timings: { libraryLoadMs: 960, detailSwitchMs: 64, quickSearchMs: 190, searchMs: 770 },
    renderedRows: { initial: 240, afterLoadMore: 480 },
  };
  ...
  assert.deepEqual(second.delta.timings, { libraryLoadMs: 60, detailSwitchMs: -6, quickSearchMs: -20, searchMs: -30 });
...
test('large library smoke history flags substantial timing regressions without failing the run', () => {
  ...
  const baselineReport = {
    gameCount: 4500,
    timings: { libraryLoadMs: 1000, detailSwitchMs: 80, quickSearchMs: 300, searchMs: 800 },
    renderedRows: { initial: 240, afterLoadMore: 480 },
  };
  const slowerReport = {
    gameCount: 4500,
    timings: { libraryLoadMs: 1700, detailSwitchMs: 90, quickSearchMs: 900, searchMs: 1400 },
    renderedRows: { initial: 240, afterLoadMore: 480 },
  };
  ...
  assert.deepEqual(result.warnings.map((warning) => warning.metric), ['libraryLoadMs', 'quickSearchMs', 'searchMs']);
...
test('large library smoke history flags rendered row regressions', () => {
  ...
  const bloatedReport = {
    gameCount: 4500,
    timings: { libraryLoadMs: 1010, detailSwitchMs: 82, quickSearchMs: 310, searchMs: 810 },
