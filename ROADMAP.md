...
- Task center shows queue progress, active/attention/completed counts with shortcut filters, elapsed/remaining time estimates, text search, status/type filters, searchable expandable per-task logs, and retry for failed/cancelled scan, path check, metadata batch match, database backup, save backup, save restore, and library archive tasks.
