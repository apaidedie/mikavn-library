...
- Library bulk selection for updating play status, favorite, hidden flags, and collection membership across the current filtered set.
...
- Maintenance duplicate game merge suggests a keep target before previewing safe migration of related records.
- Basic settings.
...
- Batch metadata updates with queue search/provider-gap filters, result status/write-state filters, and filtered bulk writeback for reviewing large match result sets.
