...
Record `Lower-version updater rehearsal` in `RELEASE_VALIDATION_REPORT.md` with `previous version: <version>. current version: <version>.` after installing the previous version, updating through the in-app updater to the current version, restarting, and verifying app-data is still intact.
