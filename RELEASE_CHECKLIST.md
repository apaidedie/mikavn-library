...
- Run `npm run release:signing:certificate:check` and record the trusted public release certificate candidate count or preflight risks in `RELEASE_VALIDATION_REPORT.md`.
...
- Run `npm run test:release-scripts`, `npm run test:playwright-scripts`, `npm run test:updater-release`, `npm run test:diagnostic-export`, `npm run test:data-safety`, `npm run test:maintenance-image-health`, and `npm run test:library-performance` when release tooling, updater wiring, diagnostic export, data safety, image maintenance, library performance, or smoke runners change.
...
Record `Lower-version updater rehearsal` in `RELEASE_VALIDATION_REPORT.md` with `previous version: <version>. current version: <version>.` after installing the previous version, updating through the in-app updater to the current version, restarting, verifying app-data is still intact, and recording SQLite `quick_check` as ok. The same line must include `Evidence: lower-version-updater-rehearsal.json.`. Copy `lower-version-updater-rehearsal.json` into `output/release/<version>-windows-x64/` with `previousVersion`, `currentVersion`, `restartVerified: true`, `appDataVerified: true`, and `databaseQuickCheck: "ok"` so `npm run release:handoff:check` validates the rehearsal evidence instead of only checking prose.
...
After copying the release executable, installer, `SHA256SUMS.txt`, `RELEASE_VALIDATION_REPORT.md`, and `MANUAL_RISK_PASS_CHECKLIST.md` into `output/release/<version>-windows-x64/`, run `npm run release:handoff:check` to verify the handoff artifacts, checksums, signing-status documentation, and manual risk-pass checklist have not drifted. For public downloads, rerun it as `npm run release:handoff:require-public` and resolve any reported blocking risks before tagging. Public-ready mode reads the GitHub Release and compares the public installer, `.sig`, `latest.json`, `SHA256SUMS.txt`, validation report, and manual checklist names, sizes, and SHA256 digests against the local handoff directory.
...
- Before relying on the updater for daily use, install the previous version, run Settings -> 检查更新, install the current version, restart, and verify `app-data`, `mikavn.db`, `images`, `cache`, `logs`, and `save-backups` still exist.
