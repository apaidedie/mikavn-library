...
export function CoverImage({
  ...
  return (
    <div className={cn('relative overflow-hidden bg-[linear-gradient(135deg,rgb(var(--panel-strong-rgb)),rgb(var(--accent-strong-rgb))_62%,#1b2230)] shadow-inner', className)}>
      ...
      {shouldRenderImage ? (
        <img alt={alt} className={cn('relative h-full w-full object-cover', blur && 'blur-md scale-105')} decoding={decoding} fetchPriority={fetchPriority} loading={loading} src={resolved} onError={() => setFailedSrc(resolved)} />
      ) : (
