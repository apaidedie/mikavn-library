...
import {
  formatSaveBackupHistoryLoadMoreLabel,
  getSaveBackupHistoryRenderWindow,
  getSaveRestorePreviewPair,
...
export function SaveBackupHistoryPanel({ backups, loading, onCopyPath, onDeleteBackup, onLoadRestorePreview, onRestore, onReveal, restorePreviewLoadingKey, restorePreviews, selectedGame }: SaveBackupHistoryPanelProps) {
  const backupHistoryIdentity = `${selectedGame?.id ?? 'none'}\n${backups.length}\n${backups[0]?.id ?? ''}\n${backups[backups.length - 1]?.id ?? ''}`;
  const [renderState, setRenderState] = useState({ count: saveBackupHistoryInitialRenderCount, identity: backupHistoryIdentity });
  const visibleCount = renderState.identity === backupHistoryIdentity ? renderState.count : saveBackupHistoryInitialRenderCount;
  const { visibleBackups, hasMore, renderedCount, totalCount } = useMemo(
    () => getSaveBackupHistoryRenderWindow(backups, visibleCount),
    [backups, visibleCount],
  );
  const loadMoreBackups = () => {
    setRenderState((current) => ({
      count: Math.min(backups.length, (current.identity === backupHistoryIdentity ? current.count : saveBackupHistoryInitialRenderCount) + saveBackupHistoryRenderBatchSize),
      identity: backupHistoryIdentity,
    }));
  };
  ...
  return (
    <Panel>
      ...
      <PanelContent className="max-h-[calc(100vh-12rem)] space-y-2 overflow-auto pr-1">
        {backups.length === 0 ? <EmptyState className="flex min-h-[22rem] flex-col items-center justify-center py-12">还没有备份记录。</EmptyState> : (
          <>
            ...
            {hasMore && (
              <Button aria-label="加载更多备份历史" className="w-full justify-center" size="sm" variant="outline" onClick={loadMoreBackups}>
                {formatSaveBackupHistoryLoadMoreLabel(renderedCount, totalCount)}
              </Button>
            )}
          </>
        )}
