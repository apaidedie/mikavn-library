...
When preparing `RELEASE_VALIDATION_REPORT.md`, record `Source commit: <git commit>` from `git rev-parse HEAD` before staging updater handoff artifacts. Also record `Large library performance warnings: <count>` and `Topbar quick search: <ms>ms, budget <ms>ms` from `output/playwright/large-library-current/large-library-report.json` so release handoff captures timing regressions and the main search entry latency even when the smoke stays within hard budgets. Also record `Lower-version updater rehearsal: passed. previous version: <version>. current version: <version>.` with app-data verification after restart, SQLite `quick_check` ok, and `Evidence: lower-version-updater-rehearsal.json.` pointing to the JSON evidence file copied into the release handoff directory. The evidence JSON must include `previousVersion`, `currentVersion`, `restartVerified`, `appDataVerified`, and `databaseQuickCheck: "ok"`.
